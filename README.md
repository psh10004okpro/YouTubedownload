...
- 🎨 **현대적이고 깔끔한 디자인** - YouTube 스타일의 빨간색 테마
- 🖼️ **비디오 썸네일 미리보기** - 다운로드 전 썸네일 확인
- 📋 **클립보드 자동 감지** - 복사한 URL을 버튼 한 번에 붙여넣기
- 📂 **폴더 빠른 열기** - 다운로드 완료 후 바로 폴더 열기
- 📊 **실시간 진행률 표시** - 퍼센트, 속도, 남은 시간
- 💬 **이모지와 함께하는 친절한 메시지**
- 📺 **상세한 비디오 정보** - 제목, 채널, 조회수, 업로드 날짜
...
2. **URL 입력 (2가지 방법)**
   - 방법 1: YouTube URL을 직접 입력
   - 방법 2: YouTube에서 URL 복사 후 "📋 붙여넣기" 버튼 클릭

3. **비디오 정보 가져오기**
   - "🔍 정보 가져오기" 버튼 클릭
   - 왼쪽에 썸네일, 오른쪽에 상세 정보 표시
   - 제목, 채널, 길이, 조회수, 업로드 날짜, 사용 가능한 자막 확인

4. **다운로드 설정**
   - 💾 **저장 위치**: 기본 폴더 사용 또는 "📁 찾아보기"로 변경
   - 🎥 **해상도**: 드롭다운에서 원하는 해상도 선택 (최고 화질 ~ 360p)
   - 📝 **자막**: 체크박스로 자막 다운로드 활성화, 언어 코드 입력 (예: ko,en,ja)

5. **다운로드 시작**
   - "⬇️ 다운로드 시작" 버튼 클릭 (크고 빨간 버튼)
   - 진행 상태 바에서 실시간 진행률, 속도, 남은 시간 확인
   - 로그에서 모든 작업 내역 확인

6. **다운로드 완료 후**
   - "📂 폴더 열기" 버튼으로 다운로드된 파일 바로 확인
